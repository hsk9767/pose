...
def train(config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        loss = criterion.forward_(output, target, target_weight)
        loss += criterion(output, target, target_weight)
        # if i % config.PRINT_FREQ == 0:
        #     gradcam_loss, gradcam_gt, gradcam_pred = criterion.get_gradcam_loss_partly(
        #         model, output, target_wider, target_weight, visualize=True)
        # else:
        #     gradcam_loss = criterion.get_gradcam_loss_partly(
        #         model, output, target_wider, target_weight, visualize=False)

        # loss += gradcam_loss

        # second input for gain
        # _, second_input, _ = get_masked_image(input, output, target_weight)
